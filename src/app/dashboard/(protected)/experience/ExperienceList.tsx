...
function formatDate(date: Date | string | null) {
  ...
  // Stable format that matches on both server and client
...
export function ExperienceList({ initialExperiences }: ExperienceListProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id && over) {
      setItems((items) => {
        ...
        // Compute new order indices (we just assign them 0, 1, 2...)
        ...
        reorderExperiencesAction(updates);
        
        // Update local orderIndex state too
  ...
  return (
    <DndContext id="dnd-experience-list" sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-white/10 text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              <SortableContext items={items.map(i => i.id)} strategy={verticalListSortingStrategy}>
                {items.map((exp) => (
                  <SortableExperienceRow 
                    ...
                    onEditSuccess={() => {
                      setEditingId(null);
                      // Form action automatically revalidates path, meaning Next.js will refetch and update initialExperiences.
                    }}
                  />
                ))}
              </SortableContext>
              {items.length === 0 && (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-zinc-500">No experience added yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <tr ref={setNodeRef} style={style} className="bg-white/5">
        <td colSpan={6} className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-semibold text-white">Edit Experience</h3>
            <button onClick={onCancelEdit} className="text-sm text-zinc-400 hover:text-white">Cancel</button>
          </div>
          <ExperienceForm initialData={exp} onSuccess={onEditSuccess} />
        </td>
      </tr>
    );
  }
  ...
  return (
    <tr ref={setNodeRef} style={style} className={`text-zinc-300 ${isDragging ? 'bg-white/10' : 'bg-transparent'}`}>
      ...
      <td className="px-4 py-3">
        <div>{exp.company}</div>
        {(exp.employmentType || exp.location || (exp.technologies && exp.technologies.length > 0)) && (
          <div className="text-xs text-zinc-500 mt-1 flex flex-col gap-0.5">
            ...
            {exp.technologies && exp.technologies.length > 0 && (
              <span className="text-[#4F8CFF]/80">{exp.technologies.join(', ')}</span>
            )}
          </div>
        )}
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex items-center justify-end gap-3">
          ...
          <form action={deleteExperienceAction}>
            ...
            <ConfirmSubmitButton message="Delete this experience?" className="inline-flex text-zinc-500 hover:text-red-400 transition-colors">
              <Trash2 size={16} />
            </ConfirmSubmitButton>
          </form>
        </div>
